...
def process_grid(path, file):
	...
	# loop through intervals and replace string punctuation
	...
	tg.append(wt)
	# print("Saving as: {}".format(os.path.join(args.outputDir, file)))
	tg.write(os.path.join(args.outputDir, file))

for root, dirs, files in os.walk(args.inputDir):
	for name in files:
		if name.endswith(".TextGrid"):
			print("Processing: {}".format(name))
			process_grid(root, name)
	print("Done!")